...
echo "=== OpenHands on klogin 一键部署 [prompt-plugin] ==="
echo ""
...
REPO_URL="${OH_PP_REPO_URL:-https://github.com/GloryyrolG/OpenHands.git}"
REPO_BRANCH="${OH_PP_REPO_BRANCH:-prompt-plugin}"

# JWT 密钥（生产建议通过 env 变量传入）
JWT_SECRET="${OH_PP_JWT_SECRET:-$(openssl rand -hex 32 2>/dev/null || echo "change-me-in-production")}"
...
if [ -n "\$CURRENT_MODEL" ]; then
    cat > ~/.openhands-pp/settings.json << EOF
{
...
# 清理旧容器
sudo docker ps -a --filter name=oh-pp- -q | xargs -r sudo docker rm -f 2>/dev/null || true
sudo docker rm -f openhands-app-pp 2>/dev/null || true

# Git clone（或更新）multiuser 分支源码
if [ -d /tmp/openhands-pp/.git ]; then
    echo ">>> 更新已有源码..."
    cd /tmp/openhands-pp
    git fetch origin
    ...
    echo ">>> 克隆源码..."
    rm -rf /tmp/openhands-pp
    git clone --depth=1 --branch ${REPO_BRANCH} ${REPO_URL} /tmp/openhands-pp
fi
echo "源码就绪 ✓"
...
sudo docker run -d \
    ...
    -e OH_WEB_URL=http://127.0.0.1:3007 \
    ...
    -v ~/.openhands-pp:/.openhands \
    openhands-pp:latest

# 等待启动
echo "等待 OpenHands 启动..."
for i in \$(seq 1 30); do
    if sudo docker logs openhands-app-pp 2>&1 | grep -q "Uvicorn running"; then
        echo "OpenHands 启动成功 ✓"
        break
    fi
    [ "\$i" -eq 30 ] && echo "警告: 等待超时，请手动确认: sudo docker logs openhands-app-pp"
    sleep 2
done
...
klogin ingresses create openhands-pp --instance "$INSTANCE_ID" --port 3007 --access-control=false 2>/dev/null \
  && echo "ingress 创建成功 ✓" \
  || echo "ingress 已存在（可忽略，域名: https://openhands-pp.svc.${INSTANCE_ID}.klogin-user.mlplatform.apple.com）"

# 5. 建立本地 SSH 隧道并验证
echo ""
echo ">>> 建立本地隧道并验证..."
pkill -f "ssh.*-L 3008.*$INSTANCE_ID" 2>/dev/null || true
sleep 1
ssh -f -N -L 3008:127.0.0.1:3007 "$INSTANCE_ID"
sleep 2

echo "测试 API 连通性..."
HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' http://localhost:3008/api/options/models)
[ "$HTTP_CODE" = "200" ] && echo "API 连通 ✓" || echo "警告: API 返回 $HTTP_CODE"

echo "测试 agent-server 代理路由..."
PROXY_CODE=$(curl -s -o /dev/null -w '%{http_code}' http://localhost:3008/agent-server-proxy/health)
[ "$PROXY_CODE" = "200" ] && echo "agent-server 代理路由 ✓" || echo "警告: 代理路由返回 $PROXY_CODE"

echo "测试 sandbox port proxy..."
SPORT_CODE=$(curl -s -o /dev/null -w '%{http_code}' "http://localhost:3008/api/sandbox-port/8001/")
...
echo "测试新建 V1 会话..."
CONV_V1_RESP=$(curl -s -X POST http://localhost:3008/api/v1/app-conversations \
...
if [ -n "$CONV_V1_ID" ]; then
    for i in $(seq 1 40); do
        STATUS_INFO=$(curl -s "http://localhost:3008/api/conversations/$CONV_V1_ID" | \
    ...
    API_KEY=$(curl -s "http://localhost:3008/api/conversations/$CONV_V1_ID" | \
    ...
    if [ -n "$API_KEY" ]; then
        SSE_FIRST=$(curl -s -N --max-time 5 \
          ...
          "http://localhost:3008/api/proxy/events/$CONV_V1_ID/stream?resend_all=true&session_api_key=$API_KEY" \
          2>/dev/null | head -2)
...
echo ""
echo "访问方式："
echo "  域名（推荐）: https://openhands-pp.svc.${INSTANCE_ID}.klogin-user.mlplatform.apple.com"
echo "  本地隧道:     http://localhost:3008  (隧道已在后台运行)"
echo ""
echo "JWT_SECRET: ${JWT_SECRET}"
echo "  → 保存此密钥！重新部署时通过 OH_PP_JWT_SECRET 环境变量传入，确保登录状态不失效"
echo "========================================"
