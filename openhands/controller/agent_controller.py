        ...
        if action.source == EventSource.USER:
            ...
            if action.wait_for_response:
                if action.content.strip():
                    ...
                    # Empty-content messages are generated by models (e.g. Gemini Flash) as a
                    # no-op between tool calls. Inject an ErrorObservation to nudge the model
                    # to take the next action rather than sending another empty message.
                    self.event_stream.add_event(
                        ErrorObservation(
                            content='No tool was called. You must use a tool to continue — '
                            'do not send empty messages. Call the appropriate tool for the next step.'
                        ),
